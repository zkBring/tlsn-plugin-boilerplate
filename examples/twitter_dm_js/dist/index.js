...
function start() {
  if (!isCorrectUrl(Config.get("tabUrl"))) {
    (0, import_hf.redirect)("https://x.com/messages");
...
function two() {
  ...
  if (!conversation_id || !cookies.auth_token || !cookies.ct0 || !headers["x-csrf-token"] || !headers["authorization"]) {
    (0, import_hf.outputJSON)(false);
    return;
  }
...
function three() {
  ...
  if (!params) {
    ...
    const id = (0, import_hf.notarize)(params);
